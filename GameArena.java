...
public class GameArena extends JPanel
    ...
    public class GameArena extends JPanel
         ...
         * @param width The width of the playing area, in pixels.
         * @param height The height of the playing area, in pixels.
         * @param createWindow Defines if a window should be created to host
         *     this GameArena. @see getPanel.
         */
        public GameArena(int width, int height, boolean createWindow) {
            this.init(width, height, createWindow);
        }
        ...
        public void setSize(int width, int height) {
            ...
            if (frame != null)
                frame.setSize(arenaWidth + frame.getInsets().left +
                                  frame.getInsets().right,
                              arenaHeight + frame.getInsets().top +
                                  frame.getInsets().bottom);
        }

        /**
         * Set the background of the GameArena window to the specified image
         * file. The image will be scaled to fit the size of the GameArena
         *
         * @param filename the filename of an image to use.
         */
        public void setBackgroundImage(String filename) {
        ...
        /**
         * Retrieves the JPanel on which this gameArena is drawn, so that it can
         * be integrated into a users application.
         *
         * n.b. This should only be called if this GameArena was constructed
         * without its own JFrame
         *
         * @return the JPanel containing this GameArena.
         */
        public JPanel getPanel() {
        ...
        public void paint(Graphics gr) {
            ...
            if (!rendered) {
                ...
                // Create a buffer the same size of the window, which we can
                // reuse from frame to frame to improve performance.
                buffer = new BufferedImage(arenaWidth, arenaHeight,
                                           BufferedImage.TYPE_INT_ARGB);
                graphics = buffer.createGraphics();
                graphics.setRenderingHints(renderingHints);
            ...
            if (frame == null) {
                // Find the JFrame we have been added to, and attach a
                // KeyListner
                frame = (JFrame)SwingUtilities.getWindowAncestor(this);
            ...
            synchronized (this) {
                if (!this.exiting) {
                    ...
                    if (backgroundImage != null)
                        graphics.drawImage(
                            backgroundImage, 0, 0, arenaWidth, arenaHeight, 0,
                            0, backgroundImage.getWidth(null),
                            backgroundImage.getHeight(null), null);
                    ...
                    for (Object o : things) {
                        ...
                        if (o instanceof Rectangle) {
                            ...
                            graphics.translate(r.getXPosition(),
                                               r.getYPosition());
                            graphics.rotate(Math.toRadians(r.getRotation()));

                            graphics.fillRect(
                                (int)(-r.getXOrigin()), (int)(-r.getYOrigin()),
                                (int)r.getWidth(), (int)r.getHeight());
                        ...
                        if (o instanceof Line) {
                            ...
                            if (l.getArrowSize() > 0) {
                                ...
                                ex = sx + ((ex - sx) * arrowRatio);
                                ey = sy + ((ey - sy) * arrowRatio);
                                graphics.fillPolygon(l.getArrowX(),
                                                     l.getArrowY(), 3);
                            }
                            graphics.draw(new Line2D.Float(sx, sy, ex, ey));
                        }
        ...
        //
        // Shouldn't really handle colour this way, but the student's haven't
        // been introduced to constants properly yet, hmmm....
        //
        private Color getColourFromString(String col) {
            Color c = colours.get(col.toUpperCase());
            ...
            synchronized (this) {
                if (things.size() > 100000) {
                    ...
                    System.out.println(
                        ...
                        + "***************** ");
                    System.out.println(
                        " ***** Only 100000 Objects Supported per "
                        + "Game Arena! ***** ");
                    System.out.println(
                        " ****************************************"
                        + "***************** ");
        ...
        /**
         * Adds a given Rectangle to the GameArena.
         * Once a rectangle is added, it will automatically appear on the
         * window.
         *
         * @param r the rectangle to add to the GameArena.
         */
        ...
        /**
         * Adds a given Text object to the GameArena.
         * Once a Text object is added, it will automatically appear on the
         * window.
         *
         * @param t the text object to add to the GameArena.
         */
        ...
        /**
         * Remove a Text object from the GameArena.
         * Once a Text object is removed, it will no longer appear on the
         * window.
         *
         * @param t the text object to remove from the GameArena.
         */
        ...
        /**
         * Removes every object that has ever been added to the GameArena.
         * Nothing should appear on the GameArena window after this has
         * executed.
         */
        public void clearGameArena() {
            synchronized (this) {
        ...
        /**
         * Pause for a 1/50 of a second.
         * This method causes your program to delay for 1/50th of a second.
         * You'll find this useful if you're trying to animate your application.
         *
         */
        public void pause() {
            try {
        ...
        /**
         *	Internal method to check if ASCII code is A-Z.
         *	@param asciiVal the ascii character value to check if it is
         *within A-Z ascii.
         *	@return true or false if the ascii character value is A-Z ascii.
         */
        private boolean isAsciiAZ(int asciiVal) {
        ...
        /**
         *	Check if an ASCII A-Z or a-z letter key has been pressed.
         *	@param key the ASCII A-Z or a-z char to check if it has been
         *pressed.
         * 	@return whether or not that char has been pressed.
         */
        public boolean letterPressed(char key) {
        ...
        /**
         * Gathers location informaiton on the mouse pointer.
         * @return the current X coordinate of the mouse pointer in the
         *     GameArena.
         */
        public int getMousePositionX() {
            return mouseX;
        ...
        /**
         * Gathers location informaiton on the mouse pointer.
         * @return the current Y coordinate of the mouse pointer in the
         *     GameArena.
         */
        public int getMousePositionY() {
            return mouseY;
